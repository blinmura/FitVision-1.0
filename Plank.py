...
class Plank(Exercise):
    ...
    def exercise(self, source):
        ...
        eang1 = 0
        plankTimer = None
        plankDuration = int(ProgressManager.load_progress().get("Plank", 0)) # <--- загружаем сохранённый прогресс
        ...
        while True:
            ...
            try:
                # Если тело почти прямое — запускаем таймер
                if eang1 > 170:
        ...
        ProgressManager.save_progress("Plank", round(plankDuration))  # <--- сохраняем время при выходе
        pose.close()
