...
#            return json.load(f).get("Squat", 0)
#    return 0

#def save_progress(count):
#    progress = {"Squat": count}
...
class Squat(Exercise):
    ...
    def exercise(self, source):
        print("Squat: Запуск обработки")
        threaded_camera = ThreadedCamera(source)
        print("Squat: Камера инициализирована")
        if not threaded_camera.capture.isOpened():
            print("Ошибка: Камера не открылась!")
            return
        scount = int(ProgressManager.load_progress().get("Squat", 0))

        while True:
            success, image = threaded_camera.show_frame()
            print(f"Кадр получен: {success}, {image is not None}")
            ...
            try:
                ...
                if abs(hip_coord[1] - knee_coord[1]) > 35 and performedSquat:
                    scount += 1
                    performedSquat = False
                    ProgressManager.save_progress("Squat", scount)
